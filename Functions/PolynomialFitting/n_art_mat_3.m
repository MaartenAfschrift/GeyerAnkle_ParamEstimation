    ...
    for n_q2 = n_q2s
        ...
        for n_q3 = n_q3s
            ...
            for n_q4 = n_q4s
                ...
                for dof_nr = 1:n_dof
                    if dof_nr ==1
                        diff_mat_qT = n_q1*q(:,1).^(n_q1-1).*q(:,2).^n_q2.*q(:,3).^n_q3.*q(:,4).^n_q4;
                    elseif dof_nr ==2
                        diff_mat_qT = q(:,1).^n_q1.*n_q2.*q(:,2).^(n_q2-1).*q(:,3).^n_q3.*q(:,4).^n_q4;
                    elseif dof_nr == 3
                        diff_mat_qT = q(:,1).^n_q1.*q(:,2).^n_q2.*n_q3.*q(:,3).^(n_q3-1).*q(:,4).^n_q4;
                    else
                        diff_mat_qT = q(:,1).^n_q1.*q(:,2).^n_q2.*q(:,3).^n_q3.*n_q4.*q(:,4).^(n_q4-1);
                    end
                    diff_mat_q(:,coeff_nr,dof_nr) = diff_mat_qT;
                end
                %
                coeff_nr = coeff_nr + 1;
            end

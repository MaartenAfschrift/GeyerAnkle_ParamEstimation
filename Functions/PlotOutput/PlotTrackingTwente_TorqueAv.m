function [h1] = PlotTrackingTwente_TorqueAv(datafile,nUnp,nP,Mag,varargin)
...
for i=1:3
    subplot(1,3,i);
    plot(AllDat(:,1,i),'Color',Cols(1,:),'LineWidth',lw*2); hold on;
    for j=2:length(iP_Unique)
        plot(AllDat(:,j,i),'Color',Cols(j,:),'LineWidth',lw); hold on;
    end
    plot(AllDat(:,1,i),'Color',Cols(1,:),'LineWidth',lw*2); hold on;
    set(gca,'YLim',[-100 50]);
